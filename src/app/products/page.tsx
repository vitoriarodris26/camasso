  ...
  return (
    <section className="bg-gray-50 py-20 relative" id="produtos">
      <h1 className="text-2xl font-bold text-center mb-16 text-gray-900">
        Produtos que você encontra na <span className="text-green-600">Camasso</span>
      </h1>
          ...
          {categoriesList.map((cat) => (
            <button
              ...
              className={`pb-4 text-sm font-medium transition-all whitespace-nowrap relative px-2
                ${activeTab === cat ? 'text-green-600 font-bold' : 'text-gray-500 hover:text-green-600'}`}
            >
              {cat}
              {activeTab === cat && (
        ...
        <div key={activeTab} className="flex flex-col gap-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-auto lg:h-[400px]">
            ...
            <article className="bg-gray-50 rounded-[2rem] p-8 md:p-12 flex flex-col justify-center items-center text-center border border-gray-100">
              ...
              <h2 className="text-3xl md:text-4xl font-normal text-gray-900 mb-4">{content.highlightTitle}</h2>
              <p className="text-gray-500 mb-8 max-w-md">{content.highlightDesc}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {content.items.map((item, index) => (
              ...
              >
                <div className="relative w-full flex-1 mb-4 flex items-center justify-center">
                   <Image src={item.image} alt={item.title} width={150} height={100} className="object-contain group-hover:scale-110 transition-transform" />
                </div>
                <div className="flex items-end justify-between">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 mb-3">{item.title}</h3>
                    <div className="flex gap-2">
                      {item.tags.map(tag => <span key={tag} className="bg-gray-100 text-gray-500 text-[10px] font-bold px-3 py-1 rounded-full">{tag}</span>)}
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
          ...
          <div className="relative bg-white w-full max-w-4xl max-h-[90vh] rounded-[2.5rem] overflow-hidden shadow-2xl animate-scale-in flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/2 p-8 md:p-12 overflow-y-auto">
              ...
              <span className="text-green-600 font-bold text-xs uppercase tracking-widest">{activeTab}</span>
              <h3 className="text-3xl font-bold text-gray-900 mt-2 mb-6">{selectedProduct.title}</h3>
              
              <div className="space-y-4 text-gray-600">
                <p>Equipamento de alta performance ideal para soluções hospitalares e industriais, garantindo durabilidade e precisão na identificação.</p>
                
                <ul className="space-y-2 mt-6">
                   <li className="flex items-center gap-2">✅ Qualidade de impressão térmica</li>
                   <li className="flex items-center gap-2">✅ Alta durabilidade e performance</li>
                   <li className="flex items-center gap-2">✅ Homologado para setor hospitalar</li>
                </ul>
              </div>
